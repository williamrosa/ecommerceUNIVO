...
class MarcasController extends Controller
{
            ...
            try {
                ...
                if(empty($imagen)){
                    ...
                    if(!$image->saveAs($path)){
                        throw new Exception(implode("<br />", \yii\helpers\ArrayHelper::getColumn($image->getErrors(),0,false)));
                    }
                }
    ...
    public function actionUpdate($id_marca)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try {
                $image = UploadedFile::getInstance($model, 'imagen');
                $imagenDb = $_POST['Marcas']['imagenDb'];

                if (empty($imagenDb) && empty($image)) {
                ...
                if(!empty($image)){
                    ...
                    if (!empty($imagenDb)) {
                        $ruta_borrar = Yii::$app->basePath . '/web'.$imagenDb;
                        unlink($ruta_borrar);
                    }
                }
    ...
    public function actionDelete($id_marca)
    {
        ...
        try {
            $model->estado = 0; 
            $data_original = Json::encode($model->getOldAttributes(),
            JSON_PRETTY_PRINT);
            $data_modificada = Json::encode($model->getDirtyAttributes(),
             JSON_PRETTY_PRINT);
